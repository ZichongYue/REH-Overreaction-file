...
log using ExpInflation2026_0325, replace

use dta/panel_data, clear
xtset panelid Year

gen Experienced_inflation_F = F.Experienced_inflation
gen ps = Experienced_inflation_F /* ps: public signal */
...
outreg2 using ExpInflation.xls, replace dec(3) ///
ctitle(FE) keep (ps)
...
outreg2 using ExpInflation.xls, append dec(3) ///
ctitle(FE) keep (ps DHiI)
*
...
* Lagged zDC
gen zDC_lag = L.zDC 
reghdfe Depvar c.ps##i.zDC_lag DHiI, absorb(panelid) vce(cluster panelid)
outreg2 using ExpInflation.xls, append dec(3) ///
ctitle(FE) keep (ps DHiI 1.zDC_lag c.ps#1.zDC_lag)
...
reghdfe Depvar c.ps##i.FLgroup1_lag DHiI, absorb(panelid) vce(cluster panelid)
outreg2 using ExpInflation.xls, append dec(3) ///
ctitle(FE) keep (ps DHiI 1.FLgroup1_lag c.ps#1.FLgroup1_lag)
...
reghdfe Depvar c.ps##i.SCgroup1_lag DHiI, absorb(panelid) vce(cluster panelid)
outreg2 using ExpInflation.xls, append dec(3) ///
ctitle(FE) keep (ps DHiI 1.SCgroup1_lag c.ps#1.SCgroup1_lag)
...
reghdfe Depvar c.ps##c.price_z_lag DHiI, absorb(panelid) vce(cluster panelid)
outreg2 using ExpInflation.xls, append dec(3) ///
ctitle(FE) keep (ps DHiI price_z_lag c.ps#c.price_z_lag)
lincom _b[ps] + 2*_b[c.price_z_lag#c.ps] /* Note here we assume that individual become more risk-averse by 2sd */


* (3)-1, 2, 3, 4, 5, 6: Do present bias, planning ability, risk attitude (risk aversion) self-control matter for reaction? 
...
reghdfe Depvar c.ps##i.zDC_lag ///
		 ...
		 DHiI, absorb(panelid) vce(cluster panelid)	 
outreg2 using ExpInflation.xls, append dec(3) ///
...
* (3)-1, 2, 3, 4, 5, 6 with control variables
reghdfe Depvar c.ps##i.zDC_lag ///
         i.FLgroup1_lag c.ps#i.FLgroup1_lag ///
		 i.SCgroup1_lag c.ps#i.SCgroup1_lag ///
		 price_z_lag c.ps#c.price_z_lag ///
...
lincom _b[ps] + _b[1.zDC_lag#c.ps] + _b[1.FLgroup1_lag#c.ps] + _b[1.SCgroup1_lag#c.ps] ///
                    + 2*_b[c.price_z_lag#c.ps]
					
					

erase ExpInflation.txt
