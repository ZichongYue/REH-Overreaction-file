...
outreg2 using NoHiinfdummy.xls, replace dec(3) ///
ctitle(CFE) keep (ps)
...
reghdfe Depvar ps, absorb(panelid) vce(cluster panelid)
outreg2 using NoHiinfdummy.xls, append dec(3) ///
ctitle(CFE) keep (ps)
*
...
*
* (3)-1-a: Does dynamic consistency matter for reaction? 
...
* Lagged zDC
gen zDC_lag = L.zDC 
reghdfe Depvar c.ps##i.zDC_lag, absorb(panelid) vce(cluster panelid)
outreg2 using NoHiinfdummy.xls, append dec(3) ///
ctitle(CFE) keep (ps 1.zDC_lag c.ps#1.zDC_lag)
...
reghdfe Depvar c.ps##i.FLgroup1_lag, absorb(panelid) vce(cluster panelid)
outreg2 using NoHiinfdummy.xls, append dec(3) ///
ctitle(CFE) keep (ps 1.FLgroup1_lag c.ps#1.FLgroup1_lag)
...
reghdfe Depvar c.ps##i.SCgroup1_lag, absorb(panelid) vce(cluster panelid)
outreg2 using NoHiinfdummy.xls, append dec(3) ///
ctitle(CFE) keep (ps 1.SCgroup1_lag c.ps#1.SCgroup1_lag)
...
reghdfe Depvar c.ps##c.price_z_lag, absorb(panelid) vce(cluster panelid)
outreg2 using NoHiinfdummy.xls, append dec(3) ///
ctitle(CFE) keep (ps price_z_lag c.ps#c.price_z_lag)
